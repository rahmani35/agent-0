...
export function AuthProvider({ children }) {
  const [user, setUser] = useState(() => {
    const saved = localStorage.getItem('agent0_user');
  ...
  useEffect(() => {
    async function initAuth() {
      ...
      if (profile) {
        setUser(profile);
        localStorage.setItem('agent0_user', JSON.stringify(profile));
      } else {
        setUser(null);
      }
      setLoading(false);
  ...
  const loginWithGoogle = async (credential) => {
    ...
    localStorage.setItem('agent0_google_token', credential);
    localStorage.setItem('agent0_user', JSON.stringify(data.user));
    setUser(data.user);
    return data.user;
  };
